...
import { Volume2, VolumeX, RotateCcw, Swords, Shield, Award } from 'lucide-react';
...
export const CyberBrawlerGame: React.FC<ArcadeGameProps> = ({ onExit, machineName }) => {
  ...
  return (
    <div className="w-full h-full flex flex-col bg-neutral-950 text-white font-mono select-none overflow-hidden">
      {/* Header */}
      <div className="bg-gradient-to-r from-amber-900 via-neutral-900 to-amber-950 border-b-2 border-amber-500/70 px-6 py-3 flex items-center justify-between shadow-lg">
        <div className="flex items-center gap-3">
          <Swords className="w-6 h-6 text-amber-400 animate-pulse" />
          <div>
            <h1 className="text-xl font-black text-amber-300 italic tracking-wider">
              CYBER PUNCHOUT <span className="text-xs px-2 py-0.5 rounded bg-amber-600/30 text-amber-200 border border-amber-500/50">ARCADE BRAWLER</span>
            </h1>
            <p className="text-xs text-neutral-400">Title Bout • Heavyweight Championship</p>
          </div>
        </div>

        <div className="flex items-center gap-4">
          <div className="text-xs text-neutral-300">
            SCORE: <span className="text-amber-300 font-bold">{score}</span> | COMBO: <span className="text-rose-400 font-bold">{combo}x</span>
          </div>
          <button
            onClick={resetFight}
            className="px-3 py-1.5 rounded bg-amber-700 hover:bg-amber-600 text-xs font-bold transition flex items-center gap-1"
          >
      ...
      {/* Ring Stage */}
      <div className="flex-1 flex flex-col items-center justify-center p-6 relative bg-[radial-gradient(ellipse_at_top,rgba(245,158,11,0.15),transparent_70%)]">
        {/* Health Bars */}
        <div className="w-full max-w-2xl flex items-center justify-between gap-8 mb-6">
          <div className="flex-1">
            <div className="flex justify-between text-xs font-bold mb-1">
              <span className="text-cyan-400">PLAYER (CHALLENGER)</span>
              <span className="text-neutral-300">{playerHp}%</span>
            </div>
            <div className="h-4 bg-neutral-800 rounded-full overflow-hidden border border-cyan-500/40">
              <div
          ...
          <div className="px-3 py-1 bg-amber-950/80 rounded-lg border border-amber-500 text-amber-300 font-black text-sm">
            VS
          </div>
          ...
          <div className="flex-1 text-right">
            <div className="flex justify-between text-xs font-bold mb-1">
              ...
              <span className="text-rose-400">CYBER MOE (CHAMPION)</span>
            </div>
            <div className="h-4 bg-neutral-800 rounded-full overflow-hidden border border-rose-500/40">
              <div
        ...
        <div className="px-6 py-1.5 rounded-full bg-neutral-900/90 border border-amber-500/60 text-amber-300 font-black text-sm mb-6 shadow-lg tracking-wider animate-pulse">
          {combatText}
        </div>

        {/* The Boxing Ring Visual Canvas */}
        <div className="relative w-full max-w-xl h-72 rounded-2xl border-4 border-amber-600/40 bg-gradient-to-b from-neutral-900 via-neutral-950 to-amber-950/30 flex items-center justify-center overflow-hidden shadow-2xl">
          {/* Ring Ropes */}
          <div className="absolute inset-x-0 top-16 h-1 bg-red-600/60 shadow-[0_0_8px_#ef4444]" />
          <div className="absolute inset-x-0 top-28 h-1 bg-blue-600/60 shadow-[0_0_8px_#3b82f6]" />
          <div className="absolute inset-x-0 top-40 h-1 bg-white/40" />
          ...
          >
            <div className="w-24 h-24 rounded-2xl bg-gradient-to-b from-amber-600 to-amber-800 border-2 border-amber-400 flex items-center justify-center text-4xl shadow-xl">
              {opponentStance === 'hit' ? '😵' : opponentStance === 'windup' ? '😡' : opponentStance === 'punch' ? '💥' : opponentStance === 'block' ? '🛡️' : '🥊'}
            </div>
            <div className="mt-2 text-xs font-bold text-neutral-300 bg-neutral-900/80 px-2 py-0.5 rounded border border-neutral-700">
              {opponentStance.toUpperCase()}
            </div>
          </div>

          {/* Player Gloves in Foreground */}
          <div className="absolute bottom-2 inset-x-0 flex justify-between px-16 pointer-events-none">
            <div
              className={`text-5xl transition-all duration-100 ${
                playerStance === 'punch_left'
                  ? '-translate-y-16 translate-x-8 scale-125'
                  : playerStance === 'block'
        ...
        <div className="mt-6 flex flex-wrap gap-3 justify-center">
          <button
            ...
            className="px-4 py-2 rounded-xl bg-cyan-700 hover:bg-cyan-600 font-bold text-xs shadow-md active:scale-95"
          >
            Left Jab [J]
          </button>
          <button
          ...
          <button
            onClick={handleBlock}
            className="px-4 py-2 rounded-xl bg-indigo-700 hover:bg-indigo-600 font-bold text-xs shadow-md active:scale-95 flex items-center gap-1.5"
          >
          ...
          <button
            ...
            className="px-4 py-2 rounded-xl bg-neutral-800 hover:bg-neutral-700 font-bold text-xs shadow-md active:scale-95"
          >
            Dodge Left [A]
          </button>
          <button
