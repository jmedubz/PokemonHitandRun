...
export const ModularCustomGameSlot: React.FC<ArcadeGameProps> = ({ onExit, machineName }) => {
  ...
  return (
    <div className="w-full h-full flex flex-col bg-neutral-950 text-white font-mono select-none overflow-y-auto">
      {/* Marquee */}
      <div className="bg-gradient-to-r from-purple-950 via-neutral-900 to-indigo-950 border-b border-purple-500/60 px-2 py-1 sm:px-4 sm:py-1.5 flex items-center justify-between shadow-md shrink-0">
        <div className="flex items-center gap-1.5 sm:gap-2 truncate">
          <Sparkles className="w-4 h-4 text-purple-400 animate-spin shrink-0" />
          <div className="truncate">
            <h1 className="text-xs sm:text-sm font-black text-purple-300 tracking-wider flex items-center gap-1.5 truncate">
              {machineName.toUpperCase()} <span className="hidden xs:inline text-[9px] px-1 py-0.2 rounded bg-purple-600/30 text-purple-200 border border-purple-500/50">PORT</span>
            </h1>
            <p className="hidden sm:inline-block text-[10px] text-neutral-400">Modular Import Slot</p>
          </div>
        </div>
        ...
        <button
          ...
          className="h-7 sm:h-8 px-2.5 rounded-lg bg-purple-700 hover:bg-purple-600 active:bg-purple-800 text-xs font-black transition shadow flex items-center gap-1 cursor-pointer"
        >
