...
export const RetroHitAndRunGame: React.FC<ArcadeGameProps> = ({ onExit, machineName }) => {
  ...
  return (
    <div className="w-full h-full flex flex-col bg-neutral-950 text-white font-mono select-none overflow-hidden">
      {/* Header */}
      <div className="bg-gradient-to-r from-amber-950 via-neutral-900 to-rose-950 border-b-2 border-amber-500/70 px-3 sm:px-6 py-2 flex items-center justify-between shadow-lg shrink-0">
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <div>
            <h1 className="text-sm sm:text-lg font-black text-amber-300 tracking-wider">
              RETRO HIT & RUN <span className="text-[10px] px-1.5 py-0.5 rounded bg-amber-600/30 text-amber-200 border border-amber-500/50">TURBO</span>
            </h1>
            <p className="text-[10px] text-neutral-400">
              Stage {stage} of 3 • {stage === 1 ? 'Springfield Highway' : stage === 2 ? 'Nuclear Power Grid' : 'Neon Downtown Pursuit'}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 sm:gap-4 text-xs">
          <div className="text-neutral-200">
            MPH: <span className="text-amber-400 font-black">{speedMph}</span>
          </div>
          <div className="text-neutral-200">
            TIME: <span className={`${timeLeft < 15 ? 'text-rose-500 animate-pulse' : 'text-emerald-400'} font-black`}>{timeLeft}s</span>
          </div>
